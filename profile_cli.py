...
def _format_suit_range_line(label: str, r: SuitRange, indent: str = "") -> str:
    return (
        f"{indent}{label}: cards {r.min_cards}–{r.max_cards}, "
        f"HCP {r.min_hcp}–{r.max_hcp}"
    )
...
def _format_standard_constraints_lines(std: StandardSuitConstraints, indent: str = "") -> list[str]:
    lines: list[str] = []
    lines.append(
        f"{indent}Total HCP: {std.total_min_hcp}–{std.total_max_hcp} (for entire hand)"
    )
    lines.append(_format_suit_range_line("Spades", std.spades, indent + "  "))
    lines.append(_format_suit_range_line("Hearts", std.hearts, indent + "  "))
    lines.append(_format_suit_range_line("Diamonds", std.diamonds, indent + "  "))
    lines.append(_format_suit_range_line("Clubs", std.clubs, indent + "  "))
    return lines


def _format_random_suit_constraint_lines(
    rs: RandomSuitConstraintData, indent: str = ""
) -> list[str]:
    lines: list[str] = []
    suits_txt = _fmt_suits(rs.allowed_suits)
    ...
    lines.append(f"{indent}Random Suit: {req} of {suits_txt} must meet:")
    for idx, r in enumerate(rs.suit_ranges, start=1):
        lines.append(
            f"{indent}  #{idx}: cards {r.min_cards}–{r.max_cards}, "
            f"HCP {r.min_hcp}–{r.max_hcp}"
    ...
    if rs.pair_overrides:
        lines.append(f"{indent}  Pair overrides:")
        for o in rs.pair_overrides:
            suits = getattr(o, "suits", None)
            first_range = getattr(o, "first_range", None)
            second_range = getattr(o, "second_range", None)
            ...
            if parts:
                lines.append(f"{indent}    {suits2}: " + "; ".join(parts))
            else:
                lines.append(f"{indent}    {suits2}")

    return lines


def _format_partner_contingent_constraint_lines(
    pc: PartnerContingentData, indent: str = ""
) -> list[str]:
    lines: list[str] = []
    lines.append(f"{indent}Partner Contingent constraint:")
    lines.append(f"{indent}  Partner seat: {pc.partner_seat}")
    lines.append(_format_suit_range_line("Suit", pc.suit_range, indent + "  "))
    return lines


def _format_opponent_contingent_constraint_lines(
    oc: OpponentContingentSuitData, indent: str = ""
) -> list[str]:
    lines: list[str] = []
    lines.append(f"{indent}Opponent Contingent-Suit constraint:")
    lines.append(f"{indent}  Opponent seat: {oc.opponent_seat}")
    lines.append(_format_suit_range_line("Suit", oc.suit_range, indent + "  "))
    return lines


def _format_subprofile_exclusions_lines(
    profile: HandProfile,
    seat: str,
    indent: str = "",
) -> list[str]:
    ...
    lines.append(f"{indent}Exclusions:")
    for e in relevant:
        ...
        if shapes:
            ...
            lines.append(f"{indent}  Sub-profile {idx}: exclude shapes: {shapes_txt}")
            continue
        ...
        if clauses:
            ...
            lines.append(f"{indent}  Sub-profile {idx}: exclude if: " + " AND ".join(parts))
            continue

        lines.append(f"{indent}  Sub-profile {idx}: (invalid exclusion: no shapes/clauses)")

    return lines


def _full_profile_details_text(profile: HandProfile, path: Path) -> str:
    """Return full details of a profile, including constraints, as a single text blob."""
    lines: list[str] = []
    lines.append("\n=== Full Profile Details ===")
    lines.append(f"Name        : {profile.profile_name}")
    lines.append(f"Description : {profile.description}")
    lines.append(f"Tag         : {profile.tag}")
    lines.append(f"Dealer      : {profile.dealer}")
    lines.append(f"Author      : {getattr(profile, 'author', '')}")
    lines.append(f"Version     : {getattr(profile, 'version', '')}")
    lines.append(f"File        : {path}")
    lines.append(f"Dealing ord.: {profile.hand_dealing_order}")
    ...
    for seat in profile.hand_dealing_order:
        ...
        lines.append(f"\n--- Seat {seat} ---")
        lines.append(f"Number of sub-profiles: {len(sp.subprofiles)}")
        ...
        for idx, sub in enumerate(sp.subprofiles, start=1):
            ...
            if multi:
                ...
                if weight is not None:
                    lines.append(f"    Weight: {weight:.1f}%")

            lines.append("    Standard constraints:")
            lines.extend(_format_standard_constraints_lines(sub.standard, indent="      "))

            if sub.random_suit_constraint is not None:
                lines.extend(_format_random_suit_constraint_lines(sub.random_suit_constraint, indent="    "))

            if sub.partner_contingent_constraint is not None:
                lines.extend(_format_partner_contingent_constraint_lines(sub.partner_contingent_constraint, indent="    "))

            if getattr(sub, "opponents_contingent_suit_constraint", None) is not None:
                lines.extend(_format_opponent_contingent_constraint_lines(sub.opponents_contingent_suit_constraint, indent="    "))

        lines.extend(_format_subprofile_exclusions_lines(profile, seat, indent="  "))
...
def _print_full_profile_details(profile: HandProfile, path: Path) -> None:
    ...
    print(_full_profile_details_text(profile, path), end="")
...
def view_and_optional_print_profile_action() -> None:
    ...
    if export_txt:
        ...
        text = _full_profile_details_text(profile, path)
