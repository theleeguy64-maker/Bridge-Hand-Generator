...
def _run_deal_generation_session() -> None:
    ...
    # Ask whether to use random N/S, E/W rotation for this generation only
...
def _run_profile_management() -> None:
    """
    Hand off control to the existing Profile Manager CLI in profile_cli.
    """
    print("\n=== Profile Management ===")
    # We expect profile_cli.main() to provide its own interactive loop.
    try:
        profile_cli.main()
    except AttributeError:
        print(
            "ERROR: profile_cli.main() is not defined. "
            "Please ensure profile_cli.py has a 'main()' entrypoint."
        )
...
def main() -> None:
    ...
    Entry point when running:
        ...
        python -m bridge_engine.orchestrator
    """
    _main_menu()
