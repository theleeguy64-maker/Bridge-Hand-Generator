...
def _build_subprofile(
    ...
    # ── Gate menu: choose which constraint section(s) to visit ──
    # Start from existing standard constraints, or wide-open defaults for new.
    standard = (
        existing.standard
        if existing is not None
        else StandardSuitConstraints(SuitRange(), SuitRange(), SuitRange(), SuitRange())
    )

    print("\n  Constraints:")
    print("    0) Exit (keep current / defaults)")
    ...
    if gate == 0:
        # Skip both standard and extra — return with defaults/existing
        weight_percent = existing.weight_percent if existing is not None else 0.0
        return SubProfile(
            standard=standard,
            name=name,
            random_suit_constraint=existing.random_suit_constraint if existing else None,
            partner_contingent_constraint=existing.partner_contingent_constraint if existing else None,
            opponents_contingent_suit_constraint=existing.opponents_contingent_suit_constraint if existing else None,
...
def _build_seat_profile(
    ...
    for idx in range(1, num_sub + 1):
        ...
        # Skipping preserves constraints AND exclusions unchanged.
        skipped = False
